...
int main(int argc, char** argv) {
	...
	while (fscanf(f, "%s", buffer) != EOF) {
		...
		case 2:
			...
			int out_col_width = (int)ceil(log10(matrices_A[in]->width * MAX_ABS_VAL * MAX_ABS_VAL));

			matrices_C[in]->out_col_width = out_col_width;

			char* buf = (char*)calloc(matrices_C[in]->height * (matrices_C[in]->width * out_col_width + 1) + 1, sizeof(char));
			strcpy(buf, "");
			for (int i = 0; i < matrices_C[in]->height; i++) {
				for (int j = 0; j < matrices_C[in]->width * out_col_width; j++) {
					strcat(buf, "@");
				}
				strcat(buf, "\n");
			}
			fwrite(buf, 1, (matrices_C[in]->width * out_col_width + 1) * matrices_C[in]->height, tmp);
			free(buf);
			fclose(tmp);
			break;
		}
	...
	for (int i = 0; i < proc_count; i++) {
		...
		printf("Process nr %d executed %d multiplying operations\n", children_pids[i], WEXITSTATUS(status));
	}
